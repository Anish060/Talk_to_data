...
\begin{figure}[h]
    ...
    \caption{System Architecture latest}
    \label{fig:architecture}
\end{figure}
...
\subsection{UI Redesign}
Implemented a premium black-and-white theme with modern typography, glassmorphism cards, fluid animated SVG background, and micro‑animations. Added quick‑start suggestion cards that populate the query input. Updated static asset links with cache‑busting parameter \texttt{?v=6.0}.

\subsection{Capability Validation Layer}
Introduced a three‑tier validation layer in \texttt{capability_validator.py}:
\begin{enumerate}
    \item \textbf{Catalogue Validation}: Checks query fields against the business catalogue.
    \item \textbf{Schema Validation}: Verifies column existence and data types using the Info DB metadata.
    \item \textbf{Ontology Grounding}: Ensures semantic consistency with the domain ontology.
\end{enumerate}
This layer resolves previous data relevance bugs and prevents hallucinated columns in generated SQL.
